...
import { APPCURRENCY, APPFONTMEDIUM, APPFONTREGULAR } from '../config';
					...
					<View style={layoutStyle != 'simple' ? styles.vendorDetailsWrapper : styles.vendorDetailsWrapperSimple}>
						...
						<View style={layoutStyle != 'simple' ? styles.vendorDetails : styles.vendorDetailsSimple}>
							<View style={{ flexDirection: "row", alignContent: "center", justifyContent: "flex-start" }}>
								<Text style={[layoutStyle != 'simple' ? styles.vendorName : styles.vendorNameSimple, { width: "80%" }]} numberOfLines={1}>{vendor.name}</Text>
								<View style={[{ width: "20%" }, layoutStyle == 'simple' ? { display: "none" } : {}]}>
									<Text style={[{ textAlign: 'center', color: '#fff', borderRadius: 25, fontSize: 7, padding: 2, fontWeight: 'bold' }, vendor.is_open ? { backgroundColor: "green" } : { backgroundColor: "red" }]}>
